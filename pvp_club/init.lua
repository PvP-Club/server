...
PVP.teams = {
    ...
    blue = {"TenPlusTwo", "Darkf4antom", "AnthonyDe", "JediKnight", "Gladius", "Xenon", "5uper1ach", "PantsMan", "liverpool", "The_World"},
    yellow = {"-lipop-", "minetest", "j45", "cephalotus", "Amine35", "popidog_assaillant", "Elyas_Crack", "Luis_Mi"},
    green = {"DiamondPlane", "gameit", "end", "Skyisblue", "-CrocMoney-", "N4xQ", "LuaFrank", "Code-Sploit"},
...
mt.register_on_dieplayer(function (player, reason)
	...
	if minetest.get_modpath("irc") then
		irc.say(irc.config.channel, reason.object:get_player_name() .. " has killed " .. player:get_player_name())
	end
    else
        local deaths = tonumber(ms:get_string(player:get_player_name().."deaths")) or 0
        ms:set_string(player:get_player_name().."deaths", tostring(deaths + 1))
