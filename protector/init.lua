...
local is_owner = function(meta, name)
	return PVP.get_team(name) == PVP.get_team(meta:get_string("owner"))
end
...
protector.can_dig = function(r, pos, digger, onlyowner, infolevel)
	...
	for n = 1, #pos do
		...
		if infolevel == 1 and owner ~= PVP.get_team(digger) then
			show_msg(digger,
...
minetest.register_node("protector:protect", {
	...
	after_place_node = function(pos, placer)
		...
		meta:set_string("owner", PVP.get_team(placer:get_player_name()))
